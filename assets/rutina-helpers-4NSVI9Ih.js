import{D,p as k,i as S,n as L,k as M,G as q,J as l,s as p,K as A}from"./index-0HqlbGei.js";const v={Core:"tag-core",Piernas:"tag-piernas",Pecho:"tag-pecho",Espalda:"tag-espalda",Brazos:"tag-brazos",Glúteos:"tag-gluteos",Hombros:"tag-hombros"},R={1:"Lunes",2:"Martes",3:"Miercoles",4:"Jueves",5:"Viernes",6:"Sabado",0:"Domingo"},T={1:"L",2:"M",3:"X",4:"J",5:"V",6:"S",0:"D"},C=[1,2,3,4,5,6,0];function E(s){if(!s||!s.nombre)return"";const a=s.nombre.match(/^Día \d+ [-–] (.+)$/i);return a?a[1]:s.nombre}function H(s){return s?`#${String(s).padStart(3,"0")}`:""}function I(s){return s==="gimnasio"?"🏋️":s==="cross"?"🏃":""}function N(s,a=!1){const i=[...new Set(s.circuitos.map(e=>e.grupoMuscular))],n=a?"tag-sm":"";return i.map(e=>`<span class="tag ${n} ${v[e]||""}">${e}</span>`).join("")}function G(s){const a=A(s.id);if(!a)return'<span class="rutina-last-done">Sin registros</span>';const i=new Date(a.fecha),n=Math.floor((Date.now()-i.getTime())/864e5);return`<span class="rutina-last-done">${n===0?"Hoy":n===1?"Ayer":`Hace ${n} dias`}</span>`}function j(s){const a=D(s);if(!a)return;const i=a.circuitos.map((n,e)=>`
    ...
    `).join("");k({title:a.nombre,body:`<div class="preview-body">${i}</div>`,confirmText:`${S.kettlebell} Iniciar`,cancelText:"Volver",onConfirm:()=>L(`/workout/${s}`),html:!0})}function B(s,a,i,n){const e=R[a],o=i==="gimnasio"?"cross":"gimnasio",b=o==="gimnasio"?"🏋️":"🏃",y=o==="gimnasio"?"Gimnasio":"Cross",$=i==="gimnasio"?"🏋️":"🏃",g=M().filter(t=>t.usuario===s&&t.tipo===i),u=g.find(t=>t.diaSemana===a),f=g.map(t=>`<button class="day-assign-option ${u&&t.id===u.id?"active":""}" data-assign-rutina="${t.id}">${t.nombre}</button>`).join(""),m=i==="gimnasio"?"Gimnasio":"Cross",w=`<div style="color:var(--color-text-muted);padding:var(--space-sm) 0">No hay rutinas de ${m}</div>`,h=`
